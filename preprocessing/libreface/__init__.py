...
from .detect_mediapipe_image import *
from .AU_Detection.inference import detect_action_units, detect_action_units_video
...
from .utils import get_frames_from_video_ffmpeg, uniquify_file, check_file_type
...
def get_facial_attributes_image(image_path:str, 
    ...
    if model_choice == "separate_prediction_heads":
...
def get_facial_attributes_video(video_path, 
    ...
    # Stampa il tipo di dispositivo usato (CPU o CUDA)
    print(f"Using device: {device} for inference...")
    
    # Estrae i frame dal video come immagini singole, memorizzate temporaneamente
    frames_df = get_frames_from_video_ffmpeg(video_path, temp_dir=temp_dir)
    
    # Estrae il nome del video senza estensione (serve a creare una sottodirectory dedicata)
    cur_video_name = ".".join(video_path.split("/")[-1].split(".")[:-1])
    
    # Allinea i frame del video (face alignment) e restituisce:
    # - lista di path ai frame allineati
    # - headpose stimato (yaw, pitch, roll)
    # - landmark 3D per ciascun frame
    ...
    # Se si sceglie il modello "separate_prediction_heads" (attivo nel tuo caso):
    if model_choice == "separate_prediction_heads":
        # Estrae AU binarie (presenti/non presenti) per ogni frame in batch
        detected_aus = detect_action_units_video(
        ...
        # === Inizio blocco commentato: intensità AU separata ===
        ...
        # Se il modello non è riconosciuto, interrompe l'esecuzione
        print(f"Undefined model_choice = {model_choice} for get_facial_attributes_video()")
        raise NotImplementedError

    # === Inizio blocco commentato: espressione facciale ===
    ...
    # Aggiunge le AU binarie al DataFrame dei frame
    frames_df = frames_df.join(detected_aus)

    # Aggiunge le AU intensità (se estratte - al momento vuote)
    frames_df = frames_df.join(au_intensities)

    # Aggiunge le espressioni facciali (se estratte - al momento vuote)
    frames_df = frames_df.join(facial_expression)

    # Ritorna il DataFrame completo con tutti gli attributi facciali per ogni frame
    return frames_df
