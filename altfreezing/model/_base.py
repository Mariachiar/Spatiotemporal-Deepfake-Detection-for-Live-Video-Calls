...
class ModelBase(nn.Module):
    ...
    def load(self, fullpath=None, epoch=-1, pretrained=None):
        ...
        try:
            saved = torch.load(fullpath, map_location="cpu")

            # 1) estrai uno state_dict valido: puro, "state_dict", o "classifier_state_dict"
            if isinstance(saved, dict):
                if   "state_dict" in saved: sd = saved["state_dict"]
                elif "classifier_state_dict" in saved: sd = saved["classifier_state_dict"]
                elif "model_state_dict" in saved: sd = saved["model_state_dict"]  # <- AGGIUNGI QUESTA
                else: sd = saved
            else:
                sd = saved

            # 2) rimuovi prefissi comuni
            def strip_prefix(k: str) -> str:
                for p in ("module.", "network.", "_warped_network."):
                    if k.startswith(p):
                        return k[len(p):]
                return k
            sd = {strip_prefix(k): v for k, v in sd.items()}

            # 3) filtra chiavi esistenti e shape compatibili
            param_dict = self.network.state_dict()
            loadable = {k: v for k, v in sd.items() if (k in param_dict and param_dict[k].shape == v.shape)}

            # 4) diagnostica
            redundant_key = sorted([k for k in sd.keys() if k not in param_dict])
            unmatch_key  = sorted([k for k in sd.keys() if (k in param_dict and param_dict[k].shape != sd[k].shape)])
            loaded_key   = sorted(list(loadable.keys()))
            all_key      = set(param_dict.keys())
            unfind_key   = sorted(list(all_key - set(loaded_key) - set(unmatch_key)))

            if redundant_key:
                logger.warn("{} are in checkpoint, but not found in model {}".format(set(redundant_key), self.name))
            if unfind_key:
                logger.warn("{} are in model, but not found in chekppint loaded for {}".format(set(unfind_key), self.name))
            if unmatch_key:
                logger.warn("{} have unmatching shape between checkpoint&model for {}".format(set(unmatch_key), self.name))

            # 5) applica e carica
            param_dict.update(loadable)
    ...
    def find_last(self, epoch=-1, model_dir=None):
        """Find the last checkpoint file in the model directory"""
    ...
    def config(self) -> dict:
        """
        Return private config dict. Specific config overrides defaults for current plugin
        """
